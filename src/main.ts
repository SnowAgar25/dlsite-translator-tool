import { createPreviewBox, calculatePosition, addNavLink, addTranslationTableStyles } from './ui';
...
function displayProductInfo(info: ProductCache[string], productId: string, x: number, y: number, subDomain: string): void {
  ...
  if (translationInfo.is_translation_agree) {
    ...
    content.push(`譯者報酬: ${translatorRate}%`);

    if (info.official_price) {
      const wholesalePrice = getWholesalePrice(info.official_price);
      if (wholesalePrice) {
        const predictedCompensation = Math.floor(wholesalePrice * (translatorRate / 100));
        content.push(`預測報酬: ${predictedCompensation} 日元 (基於卸価格 ${wholesalePrice} 日元)`);
      } else {
